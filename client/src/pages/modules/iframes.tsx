...
export default function Iframes() {
  ...
  const [actions, setActions] = useState<Action[]>([
    { id: 'frame1', description: 'Switched to first iframe', timestamp: new Date(), completed: false },
    { id: 'frame2', description: 'Switched to second iframe', timestamp: new Date(), completed: false }
  ]);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="bg-slate-800 p-6 mb-6 border-slate-700 hover:border-emerald-500/50 transition-colors">
          <div className="prose prose-invert max-w-none">
            <h2>Working with Iframes</h2>
            <p>Practice switching between different iframes:</p>
          </div>
        </Card>
        ...
        <div className="grid gap-6 mb-6">
          <iframe
            src="about:blank"
            className="w-full h-48 bg-slate-700 rounded border border-slate-600 hover:border-emerald-500/50 transition-all duration-300"
            onFocus={() => {
          ...
          {currentFrame && (
            <div className="p-4 bg-emerald-900/30 border border-emerald-500/50 rounded animate-fade-in">
              Currently focused: {currentFrame}
            </div>
          )}
        </div>
